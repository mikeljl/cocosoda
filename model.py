...
class Multi_Loss_CoCoSoDa(BaseModel): #check this for pre-training !!!

    def __init__(self, base_encoder, args, mlp=False):
        super(Multi_Loss_CoCoSoDa, self).__init__()

        self.K = args.moco_k #queue size
        self.m = args.moco_m #momentum arg
        self.T = args.moco_t #temperature
        dim= args.moco_dim

        # create the encoders
        # num_classes is the output fc dimension
        self.code_encoder_q = base_encoder #from hugging face model
        ...
        for param_q, param_k in zip(self.code_encoder_q.parameters(), self.code_encoder_k.parameters()):
            param_k.data.copy_(param_q.data)  # initialize by copying from query encoder
            param_k.requires_grad = False  # not update by gradient, updated by momentum
    ...
    def forward(self, source_code_q, source_code_k, nl_q,nl_k):
        ...
        if self.agg_way == "avg":
            ...
            # outputs dimension = [batch_size, seq_len, hidden_dim], the [0] extracts hidden state 
            outputs = self.code_encoder_q(source_code_q, attention_mask=source_code_q.ne(1))[0]
            #ne(1) gives tensor of [batch_size, seq_len], True for non-padding tokens, used to set embedding for padding to 0
            #sum(1) results in tensor of [batch_size, hidden_dim], then take avg
            code_q  = (outputs*source_code_q.ne(1)[:,:,None]).sum(1)/source_code_q.ne(1).sum(-1)[:,None] # None作为ndarray或tensor的索引作用是增加维度，
            code_q  = torch.nn.functional.normalize(code_q, p=2, dim=1)
            # compute query features for nl
            outputs= self.nl_encoder_q(nl_q, attention_mask=nl_q.ne(1))[0]  # queries: NxC   bs*feature_dim
        ...
        ## code vs nl, pair wise similarity, logits: NxN
