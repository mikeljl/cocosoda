...
def convert_examples_to_features_unixcoder(js,tokenizer,args):
    def convert_examples_to_features_unixcoder(js, tokenizer, args):
        """
        Converts examples into token IDs for UnixCoder model. format: [[CLS], <encoder-only>, [SEP], code..., [SEP]] 
        Returns:
            InputFeatures: An object containing the following attributes:
                - code_tokens (list): Tokenized code sequence with special tokens.
                - code_ids (list): Token IDs for the code sequence, padded to `args.code_length`.
                - nl_tokens (list): Tokenized natural language sequence with special tokens.
                - nl_ids (list): Token IDs for the natural language sequence, padded to `args.nl_length`.
                - url (str or int): URL or retrieval index associated with the example.
        """
...
class TextDataset_unixcoder(Dataset):
    def __init__(self, tokenizer, args, file_path=None, pooler=None):
        #list of InputFeatures, containing tokenized code and natural language sequences
        self.examples = []
        #list of dict, each is an example
        data = []
        n_debug_samples = args.n_debug_samples
        with open(file_path) as f:
            if "jsonl" in file_path: #using jsonl for pre-training
                for line in f:
                    line = line.strip()
                    js = json.loads(line)
                    if 'function_tokens' in js: #false for pre-training
...
def mask_tokens(inputs,tokenizer,mlm_probability):
    ...
    #input dim: [batch_size, args.code_length or args.nl_length]
    labels = inputs.clone()
    # We sample a few tokens in each sequence for masked-LM training (with probability args.mlm_probability defaults to 0.15 in Bert/RoBERTa)
    probability_matrix = torch.full(labels.shape, mlm_probability).to(inputs.device) # [batch_size, args.code_length or args.nl_length], each number is the proba

    print("probability matrix shape: {}".format(probability_matrix.shape))
    print("probability_matrix: {}".format(probability_matrix))

    #set proba to 0 for special/pad tokens, so don't select them as ones to be masked
    ...
    #will decide who will be masked,
    masked_indices = torch.bernoulli(probability_matrix).bool() #[batch_size, args.code_length or args.nl_length], each is boolean
    ...
    # [batch_size, args.code_length or args.nl_length], each is boolean, if true, then replace (80% of the chosen ones)
    indices_replaced = torch.bernoulli(torch.full(labels.shape, 0.8)).bool().to(inputs.device) & masked_indices
    print("indices_replaced shape: {}".format(indices_replaced.shape))
    ...
    inputs[indices_random] = random_words[indices_random] # [batch_size, args.code_length or args.nl_length], each is a index in the vocab
    print("random_words shape: {}".format(random_words.shape))
...
def replace_with_type_tokens(inputs,replaces,tokenizer,mlm_probability):
    ...
    #!!! new replaced inputs == original inputs, bug? !!!
    print("------------------------------------------------------------------------------")
    print("inputs shape: {}".format(inputs.shape))
    print(torch.sum(ref == inputs))


    return inputs, labels

def replace_special_token_with_type_tokens(inputs, speical_token_ids, tokenizer, mlm_probability):
    """sample locations of special tokens in inputs with mlm_probability, from those chosen, 80% will be replaced by special_token_id"""
    labels = inputs.clone()
...
def replace_special_token_with_mask(inputs, speical_token_ids, tokenizer, mlm_probability):
    """sample locations of special tokens in inputs with mlm_probability, from those chosen, 80% will be replaced by mask"""
    labels = inputs.clone()
    probability_matrix = torch.full(labels.shape,0.0).to(inputs.device)   
    probability_matrix.masked_fill_(labels.eq(speical_token_ids).to(inputs.device), value=mlm_probability)
    masked_indices = torch.bernoulli(probability_matrix).bool() # will decide who will be masked
...
def  multi_lang_continue_pre_train(args, model, tokenizer,pool):
    ...
    if "unixcoder" in args.model_name_or_path: #go here in pre-train
        train_datasets = []
        for train_data_file in args.couninue_pre_train_data_files: #/dataset/{language}/train.jsonl
            #init TextDataset_unixcoder, which tokenize each example in file, encapsulated in list of InputFeatures
    ...
    #creates list of infinite Dataloaders for each dataset(for each train.jsonl in each language), RandomSampler used to shuffle dataset
    ...
    t_total = args.max_steps #100000 for pretraining
    ...
    #finds the sampling proba for each dataset
    probs=[len(x) for x in train_datasets]
    probs=[x/sum(probs) for x in probs]
    probs=[x**0.7 for x in probs] #adjusting proba to avoid dominance of large dataset
    probs=[x/sum(probs) for x in probs]
    # global_step = args.start_step
    model.zero_grad()
    model.train()

    #track number of updates on weights
    global_step = args.start_step
    step=0
    tr_loss, logging_loss, avg_loss, tr_nb, best_mrr = 0.0, 0.0,0.0,0,-1
    #used for tracking loss in interval of eval_steps
    tr_num=0
    special_token_list = all_special_token 
    special_token_id_list = tokenizer.convert_tokens_to_ids(special_token_list)

    while True: 
        #select a dataloader, according to previously computed prob based on dataset size
        train_dataloader=np.random.choice(train_dataloaders, 1, p=probs)[0]
        # train_dataloader=train_dataloader[0]
        step+=1
        batch=next(train_dataloader) #gets next batch from infinite dataloader
        ...
        #[:, 3:] means all examples and all tokens except the first 3 tokens which are special tokens eg. cls...
        if step%4 == 0:
        ...
        tr_num+=1
        #inter_output: logits represent the similarity between the query embeddings (code_inputs and nl_inputs) and the key embeddings (code_transformations_ids and nl_transformations_ids), as well as the similarity with negative samples from the queues.
        #inter_target: These labels indicate which pairs of embeddings are positive matches (e.g., query and key from the same example) and which are negatives (e.g., query and key from different examples or the queue).
        ...
        if step == 1:
            ...
            logger.info("  Type: %s", type(inter_output))
            logger.info("  Shape: %s", inter_output.shape)
            logger.info("  Values: %s", inter_output)
            logger.info("inter_target:----------------------------------------")
            logger.info("  Type: %s", type(inter_target))
            logger.info("  Shape: %s", inter_target.shape)
            logger.info("  Values: %s", inter_target)
            logger.info("loss:----------------------------------------")
            logger.info("  Type: %s", type(loss))
            logger.info("  Shape: %s", loss.shape)
            logger.info("  Values: %s", loss)

        if args.n_gpu > 1:
            loss = loss.mean() # mean() to average on multi-gpu parallel training

        #scaling loss due to accumulation(only update weights every args.gradient_accumulation_steps steps)
        ...
        #accumulating loss in interval of eval_frequency
        tr_loss += loss.item()
        if (step+1)% args.eval_frequency==0:
            #printing the avg loss in the interval of eval_steps
...
def evaluate(args, model, tokenizer,file_name,pool, eval_when_training=False):
    ...
    model.train()    
    code_vecs=np.concatenate(code_vecs,0)  #dim: [num_codes, hidden_dim]
    nl_vecs=np.concatenate(nl_vecs,0) #dim: [num_nl, hidden_dim]

    scores=np.matmul(nl_vecs,code_vecs.T)
    
    #sort score for each row(each nl), reverse, so largest score first
    #dim = [num_nl, num_codes]
...
def create_model(args,model,tokenizer, config=None):
    ...
    if args.model_type ==  "base" :
        model= Model(model)
    elif args.model_type ==  "multi-loss-cocosoda": #used in pre-training
...
def main():
    ...
    if args.local_rank != -1:
        ...
        logger.info("Local rank: %s, world size: %s",args.local_rank, torch.distributed.get_world_size())

    
    pool = multiprocessing.Pool(cpu_cont) #create a pool of worker processes for parallel processing, cpu_count=16

    # Set seed
    set_seed(args.seed)

    #build model, using unixcoder in model_name_or_path
    if "codet5" in   args.model_name_or_path:
        ...
        config = RobertaConfig.from_pretrained(args.config_name if args.config_name else args.model_name_or_path, cache_dir=cache_dir, local_files_only=True)
        tokenizer = RobertaTokenizer.from_pretrained(args.tokenizer_name, cache_dir=cache_dir, local_files_only=True)
        model = RobertaModel.from_pretrained(args.model_name_or_path, cache_dir=cache_dir, local_files_only=True) #!!! change
    model=create_model(args,model,tokenizer,config) #creating Multi_Loss_CoCoSoDa in model.py
    ...
    # Training
    if args.do_multi_lang_continue_pre_train: #pre-train
    ...
    if args.do_eval: #not used in pre-training
    ...
    # if args.do_test: #used in pre-training
    ...
    if args.do_test: #used in pre-training
        logger.info("runnning test")
        checkpoint_prefix = 'checkpoint-best-mrr/model.bin'
        output_dir = os.path.join('./saved_models_grad_acc2_100000/cocosoda', '{}'.format(checkpoint_prefix))  
        print(output_dir)
        model.load_state_dict(torch.load(output_dir),strict=False)
        model.to(args.device)
        result=evaluate(args, model, tokenizer,args.test_data_file, pool)
